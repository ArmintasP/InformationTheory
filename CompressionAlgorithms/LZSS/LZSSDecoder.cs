    ...
    internal class LZSSDecoder
    {
        ...
        public static async Task DecodeAsync(string filePath, string outputFilePath)
        {
            ...
            while ((readBitsCount = await bitReader.ReadAtLeastAsync(buffer, minimumBytes: buffer.Length, throwOnEndOfStream: false)) > 0)
            {
                ...
                //{
                //    //is this a mistake?
                //    decodedText.RemoveRange(decodedText.Count - metadata.BitsAddedToFormLastByteCount, metadata.BitsAddedToFormLastByteCount);
                //}
        ...
        private static List<byte> Decode(byte[] buffer, byte[] history, ref int historyPos, List<byte> leftovers, LZSSMetadata metadata)
        {
            ...
            //decoding is wrong in some places - either encoder is messing up or again problesm with history indexing
            //Maybe encoder is encoding huge files wrongly? The issues seem present when file lenght > ~2500
            return decodedText;
        }

        private static char ConvertToChar(byte[] charInBits)
        {
            int result = 0;

            for (int i = 0; i < charInBits.Length; i++)
            {
